...
/** \file   example_03.cpp
    ...
    \author Created by P. Bochev, D. Ridzal and K. Peterson.
     ...
     ./Intrepid2_example_Drivers_Example_03.exe NX NY NZ verbose
    ...
    \code   ./Intrepid2_example_Drivers_Example_03.exe 10 10 10 \endcode
*/
 ...
 Kokkos::initialize();
  //Check number of arguments
   if (argc < 4) {
      ...
      std::cout <<"Usage:\n\n";
      std::cout <<"  ./Intrepid2_example_Drivers_Example_03.exe NX NY NZ verbose\n\n";
      std::cout <<" where \n";
...
// *********************************** GENERATE MESH ************************************
   ...
   // Get nodal coordinates
    //Kokkos::DynRankView<double,execution_space> nodeCoord("coordinates",numNodes, spaceDim);
    Kokkos::View<double *, execution_space> nodeCoord("coordinates", numNodes, spaceDim);

    Kokkos::View<bool *,execution_space> nodeOnBoundary("flag_boundaries",numNodes);
    //Kokkos::DynRankView<bool, execution_space> nodeOnBoundary("flag_boundaries", numNodes);
    //fill in the node coordinates and the ones on the boundary
    int inode = 0;
      ...
      for (int j=0; j<NY+1; j++) {
        for (int i=0; i<NX+1; i++) {
          ...
          if (k==0 || j==0 || i==0 || k==NZ || j==NY || i==NX){
             nodeOnBoundary(inode)=true;
          }
          else {
             nodeOnBoundary(inode)=false;
          }
          inode++;
        }
      }
...
#define DUMP_DATA
  ...
  // Element to Node map
  // TODO  generic template base function between exahedra, quad,triangle, tetrahedra and nodes (P1, P1bolla, P2, altro??)
    Kokkos::View<double *, execution_space> elemToNode("elemToNode",numElems, numNodesPerElem); 
    //numero di elementi user-defined, numNodesPerElem = definito dalla geomtria
    int ielem = 0;
    for (int k=0; k<NZ; k++) {
      for (int j=0; j<NY; j++) {
        for (int i=0; i<NX; i++) { //hardcodato su esaedri
        //itero sugli elementi, il secondo indice è il nodo dell'elemento
          //elemToNode(ielem,numNodesPerElem) = numNodesPerElem + ;
          elemToNode(ielem,0) = (NY + 1)*(NX + 1)*k + (NX + 1)*j + i; //0,7,
...
// ************************************ CUBATURE **************************************
   ...
   // Get numerical integration points and weights
    Intrepid2::DefaultCubatureFactory  quadratureFact;                                   
    int quadratureDegree = 2;
    auto hexQuadr = quadratureFact.create<execution_space,double,double>(hex_8, quadratureDegree); //Teuchos::RCP<Cubature<double> >

    int quadDim       = hexQuadr->getDimension();
    int numQuadrPoints = hexQuadr->getNumPoints();

    Kokkos::View<double *, execution_space> cubPoints("numQuadrPoints", numQuadrPoints, quadDim);
    Kokkos::View<double *, execution_space> cubWeights("quadWeights",numQuadrPoints);
    hexQuadr->getCubature(cubPoints, cubWeights);
...
// ************************************** BASIS ***************************************
   ...
   // Define basis 
     Intrepid2::Basis_HGRAD_HEX_C1_FEM<execution_space,double,double> hexHGradBasis;
     int numFieldsG = hexHGradBasis.getCardinality();
     Kokkos::DynRankView<double,execution_space> hexGVals("hexGVals",numFieldsG, numQuadrPoints); 
     Kokkos::DynRankView<double,execution_space> hexGrads("hexGrads",numFieldsG, numQuadrPoints, spaceDim); 

  // Evaluate basis values and gradients at cubature points
     hexHGradBasis.getValues(hexGVals, cubPoints, Intrepid2::OPERATOR_VALUE);
     hexHGradBasis.getValues(hexGrads, cubPoints, Intrepid2::OPERATOR_GRAD);
     ...
     *outStream << "Building stiffness matrix and right hand side ... \n\n";
     ...
     typedef Intrepid2::CellTools<execution_space> CellTools;
     typedef Intrepid2::FunctionSpaceTools<execution_space> fst;
     int numCells = 1;

     // Container for nodes
     Kokkos::DynRankView<double, execution_space> hexNodes("hexNodes", numCells, numNodesPerElem, spaceDim);
     // Containers for Jacobian
     Kokkos::DynRankView<double, execution_space> hexJacobian("hexNodes", numCells, numQuadrPoints, spaceDim, spaceDim);
     Kokkos::DynRankView<double, execution_space> hexJacobInv("hexJacobInv", numCells, numQuadrPoints, spaceDim, spaceDim);
     Kokkos::DynRankView<double, execution_space> hexJacobDet("hexJacobDet", numCells, numQuadrPoints);
     ...
     Kokkos::DynRankView<double, execution_space> weightedMeasure("weightedMeasure", numCells, numQuadrPoints);
     Kokkos::DynRankView<double, execution_space> hexGradsTransformed("hexGradsTransformed", numCells, numFieldsG, numQuadrPoints, spaceDim);
     Kokkos::DynRankView<double, execution_space> hexGradsTransformedWeighted("hexGradsTransformedWeighted", numCells, numFieldsG, numQuadrPoints, spaceDim);
     // Containers for right hand side vectors
     Kokkos::DynRankView<double, execution_space> rhsData("rhsData", numCells, numQuadrPoints);
     Kokkos::DynRankView<double, execution_space> localRHS("localRHS", numCells, numFieldsG);
     Kokkos::DynRankView<double, execution_space> hexGValsTransformed("hexGValsTransformed", numCells, numFieldsG, numQuadrPoints);
     Kokkos::DynRankView<double, execution_space> hexGValsTransformedWeighted("hexGValsTransformedWeighted", numCells, numFieldsG, numQuadrPoints);
     // Container for cubature points in physical space
     Kokkos::DynRankView<double, execution_space> physCubPoints("physCubPoints", numCells, numQuadrPoints, quadDim);
...
// ******************************* Build right hand side ************************************
      ...
      // evaluate right hand side function at physical points
       for (int nPt = 0; nPt < numQuadrPoints; nPt++){
